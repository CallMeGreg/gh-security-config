...
func TestWarningEnabled(t *testing.T) {
	...
	if !WarningEnabled() {
		t.Errorf("WarningEnabled() = false, want true for LogLevelInfo")
	}
	SetLogLevel(LogLevelWarning)
	if !WarningEnabled() {
		t.Errorf("WarningEnabled() = false, want true for LogLevelWarning")
	}
	SetLogLevel(LogLevelError)
	if WarningEnabled() {
		t.Errorf("WarningEnabled() = true, want false for LogLevelError")
	}
}
